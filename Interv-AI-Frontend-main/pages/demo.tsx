...
const InterviewQuestions = [
  {
    ...
    srcc: "https://liftoff-public.s3.amazonaws.com/JohnTechnical.mp4",
  },

  {
    questionID: 88,
    quest: "What is the difference between class and id in html",
    srcc: "https://intervai.s3.ap-south-1.amazonaws.com/class_id_html.mp4",
  },
  {
    questionID: 1,
    quest: "Describe difference between cookies and local storage",
    srcc: "https://intervai.s3.ap-south-1.amazonaws.com/cookies_local_storage.mp4?random=1",
  },
  {
    questionID: 16,
    quest: "Explain the concept of cross entropy loss",
    srcc: "https://intervai.s3.ap-south-1.amazonaws.com/cross_entropy_loss.mp4",
  },
];
...
const questions = [
  ...
  },
  {
    ...
    description: "KNN, Tensorflow, Neural Networks....",
    difficulty: "Medium",
  },
];
...
export default function DemoPage() {
  const { logout, setIntervData, intervData } = useUserContext();
  ...
  useEffect(() => {
    ...
    let qId;
    if (intCount === 0) {
      srourrc = InterviewQuestions[0].srcc;
      q = InterviewQuestions[0].quest;
      qId = InterviewQuestions[0].questionID;
    } else if (intCount === 1) {
      srourrc = InterviewQuestions[1].srcc;
      q = InterviewQuestions[1].quest;
      qId = InterviewQuestions[1].questionID;
    } else if (intCount === 2) {
      srourrc = InterviewQuestions[2].srcc;
      q = InterviewQuestions[2].quest;
      qId = InterviewQuestions[2].questionID;
    } else if (intCount === 3) {
      srourrc = InterviewQuestions[3].srcc;
      q = InterviewQuestions[3].quest;
      qId = InterviewQuestions[3].questionID;
    }
    setVideoSrc(srourrc);
  ...
  const handleDownload = async () => {
    if (recordedChunks.length) {
      ...
      const headers = {
        authorization: "97260bdcc67f4916a0eba19e73fe7089",
      };
      const uploadResponse = await axios.post(`${baseUrl}/upload`, output, {
        headers,
      });
      const uploadUrl = await uploadResponse.data.upload_url;
      ...
      const response = await axios.post(url, dataAudio, { headers: headers });

      const transcriptId = await response.data.id;
      ...
      while (true) {
        ...
        if (transcriptionResult.status === "completed") {
          ...
          const res: any = await handleSendAnswer(
            currentQuestionId,
            transcriptionResult.text
          );
          setIntervData((prev: any) => [...prev, res?.data]);
          setGeneratedFeedback(res?.data?.feedback?.feedback);
          setIsSuccess(true);
          setSubmitting(false);
          setCompleted(true);
          break;
  ...
  console.log(intervData, "intervdata");
  ...
  const handleSendAnswer = async (ques_id: any, user_Ans: any) => {
    try {
      const res = await axios.post(`${BASE_URL + "/interview/check-answer"}`, {
        question_id: ques_id,
        userAnswer: user_Ans,
      });
      return res;
          ...
          {completed ? (
            <div className="w-full flex flex-col max-w-[1080px] mx-auto mt-[10vh] overflow-y-auto pb-8 md:pb-12">
              ...
              >
                ...
                >
                  {intCount === 3 ? "Go to Report" : "Next Question"}
                </button>
              </motion.div>
              <motion.div
                initial={{ opacity: 0, y: 10 }}
            ...
            <div className="h-full w-full items-center flex flex-col mt-[10vh]">
              {recordingPermission ? (
                <div className="w-full flex flex-col max-w-[1080px] mx-auto justify-center">
                  <h2 className="text-2xl  font-semibold text-left text-[#1D2B3A] mb-3 bg-slate rounded-lg border bg-white px-6 py-4 shadow-sm focus:outline-none flex justify-between border-blue-500 ring-2 ring-blue-200">
                    {currQuestion}
                  </h2>
                  ...
                  >
                    ...
                    <div className="relative z-10 h-full w-full rounded-lg">
                      ...
                      {isVisible && !loadd && (
                        <div className="block absolute top-[10px] sm:top-[20px] lg:top-[40px] left-auto right-[10px] sm:right-[20px] md:right-10 h-[80px] sm:h-[140px] md:h-[200px] aspect-video rounded z-20">
                  ...
                  <motion.div
                    ...
                    className="flex flex-row space-x-1 mt-4 justify-between  items-center"
                  >
                    <div className="flex gap-x-2 items-center">
                      ...
                      <p className="text-[14px] font-normal leading-[20px] text-[#1a2b3b]">
                        Video is not stored on our servers, it is solely used
                        for transcription.
                      </p>
                    </div>
                  </motion.div>
                        ...
                        <div className="relative mb-2 flex h-full max-h-[200px] w-full cursor-pointer items-start justify-between rounded-lg p-2 font-medium transition duration-100">
                          ...
                          <div className="relative flex h-full flex-col overflow-hidden">
                            <div className="flex items-center text-left text-[#1a2b3b]">
                              <p>
                                {selected.name === "Web Development"
                                  ? "What is Async Await"
                                  : "What is Supervised learning"}
                              </p>
                            </div>
                            <p className="text-wrap grow font-normal text-[7px]">
                              Explain in as much detail as you can what this is
                      ...
                      <div className="group relative w-full">
                        <div className="relative mb-2 flex h-full max-h-[200px] w-full cursor-pointer items-start justify-between rounded-lg p-2 font-medium transition duration-100">
                          ...
                          <div className="relative flex h-full flex-col overflow-hidden">
                            ...
                            <p className="text-wrap grow font-normal text-[7px]">
                              What is Tensorflow ? How to use it
                            </p>
                            <div className="flex flex-row space-x-1">
                              <p className="inline-flex items-center justify-center truncate rounded-full border-[0.5px] border-gray-300 px-[3px] text-[7px] font-normal hover:bg-gray-50">
                                ML
